...
Sono state aggiunte `VideoRootUrl` e `SwfRootUrl` proprietà per l&#39; `getProperty` operazione.

Aggiunti parametri `appName` e `appVersion` parametri opzionali per `authHeader` il tracciamento dell’applicazione di chiamata. Accesso aggiunto a `ipsApiService.log`.

È stato aggiunto un `serviceUrl` param opzionale al servlet di generazione WSDL. Questo è particolarmente utile per i proxy di debug. Ad esempio: `http://<server>/scene7/webservice/IpsApi-2008-01-15.wsdl?serviceUrl=http://localhost:8081`

Operazione implementata `getZipEntries` .

Sono stati implementati intervalli di ricerca e valori di confronto digitati per le condizioni del campo di sistema.

È stata aggiunta la costante stringa del tipo di `'Asset'` risorsa, principalmente per consentire i campi metadati tra risorse.

Implementato `trashState` param for `searchAssets`.

Operazione implementata `getAssetPublishHistory` .

È stata aggiunta l&#39;intestazione `faultHttpStatusCode` SOAP opzionale per abilitare la gestione degli errori in Flex. Per Flex, utilizzate `<faultHttpStatusCode>200</faultHttpStatusCode>`. Il codice di stato predefinito per le risposte di errore è `500 (Internal Server Error)`.
...
Aggiunto flag enabled al `ImageMap` tipo e all&#39; `saveImageMap` operazione.
...
Operazione obsoleta `saveMetadataField` a favore di nuove `createMetadataField` e `updateMetadataField` operazioni.

Operazione di eliminazione `deleteAssetsParam` batch implementata.

Implementata operazione di spostamento `moveAssetsParam` batch.

Operazione implementata `deleteMetadataField` .
...
È stato aggiunto il supporto per `setImageSetMembers` l’inclusione `RenderSet` dei membri nelle `ImageSet` risorse.

Aggiunta `replaceImage` operazione.

Aggiunta `copyImage` operazione.

Aggiunta `setUrlModifier` di operazioni e `urlModifier/urlPostApplyModifier` campi per `LayerViewInfo`, `TemplateInfo`e `WatermarkInfo`.

Aggiunta `createDerivedAsset` operazione. Al momento `ownerHandle` deve fare riferimento a una risorsa Immagine e il tipo può essere `AdjustedView` o `LayerView`.

Aggiunta `createTemplate` operazione. Attualmente questo può essere chiamato per creare risorse Modello o Filigrana.

Impostazioni società IPS, `CompanySettings`, portate nell&#39;API dei servizi Web.

Aggiunto flag `excludeByproducts` del filtro all&#39; `searchAssets` operazione. Se si imposta questo flag su true, vengono eseguite `PSDlayer` immagini e immagini PDF estratti.

Aggiunta `getGenerationInfo` operazione.

Aggiunto nome `SystemMessage` proprietà all&#39; `getProperty` operazione.
...
Implementata l&#39;operazione di metadati `batchSetAssetMetadata` batch.
...
Implementazione `setImageMaps` e `setZoomTargets` operazioni.

Operazioni implementate `ViewerPreset` . I tipi riconosciuti sono:
...
È stata implementata `batchSetImageFields` un&#39;operazione per supportare l&#39;impostazione di dati utente immagine e altri campi immagine modificabili.
...
Operazione implementata `getActivePublishContexts` . Questa operazione restituisce un array di nomi di contesto di pubblicazione con server di pubblicazione attivi per la società specificata. I nomi correnti del contesto di pubblicazione sono:

* `ImageServing`
* `ImageRendering`
* `Video`

Operazione implementata `getSearchStrings` . Restituisce un array di stringhe di ricerca per la risorsa specificata.
...
È stato aggiunto il parametro internazionale facoltativo all&#39;intestazione `authHeader` SOAP per impostare le impostazioni internazionali per le operazioni API. Se questo parametro non è presente, `Accept-Language` verrà utilizzata l’intestazione HTTP. Se questa intestazione non è presente, verranno utilizzate le impostazioni internazionali predefinite per il server IPS.
...
Aggiunto `gzipResponse` flag a `authHeader`. Se non è presente, l&#39;API controllerà anche l&#39; `Accept-Encoding` intestazione HTTP.
...
* Per i campi booleani, `boolVal` può essere passato con il `Equals` comando op.
...
Sono stati aggiunti descrizione, `jobSubType`e `originalJobName` campi al `JobLog` tipo.
...
Inoltre, i seguenti campi non sono inclusi sia con `getJobLogs` che `getJobLogDetails`. Nelle versioni precedenti erano disponibili solo con `getJobLogDetails`.
...
È stato aggiunto il campo assetHandle al `JobLogDetail` tipo.

Aggiunto parametro di descrizione facoltativo a `submitJob`. Questo viene passato per il recupero in `getScheduledJobs`, `getActiveJobs`, e `getJobLogs`.

Campo di sistema SKU obsoleto. Il campo viene ignorato se viene passato come `SystemFieldCondition` a `searchAssets`.

Aggiunto `excludeAssetTypeArray` filtro a `searchAssets`.

È stato aggiunto `MaskInfo` il tipo a `Asset`.
...
Sono state aggiunte ulteriori opzioni per `UploadDirectoryJob` e `UploadUrlsJob` per controllare l’elaborazione dei file PostScript, Illustrator e PDF in modo indipendente. Tutti i processi esistenti forniranno i parametri necessari a ciascuno dei 3 oleodotti di lavorazione in modo che funzionino esattamente come avviene oggi. Il `PostScriptOptions` blocco originale viene usato per impostare l’elaborazione per i file Illustrator e EPS/PS. Facoltativamente, è possibile specificare blocchi di opzioni di file specifici per specificare l&#39;elaborazione. L’elenco delle modifiche comprende:
...
Modificata la proprietà di `webservice.gzip.response` configurazione in modo da ottenere uno dei seguenti valori:
