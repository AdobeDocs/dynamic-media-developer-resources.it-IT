...
La `countArray` restituzione è costituita da un array di `assetTypes` (tipo di dati `xsd:string`), ciascuno con un proprio campo conteggio (tipo di dati `xsd:int`), che consente la rappresentazione di più tipi di risorse per elemento della matrice.
Sintassi
...
Questo esempio di codice utilizza l&#39;handle della società come campo `getAssetCountsParam` inviato al server dei servizi Web IPS per ottenere il conteggio delle risorse.
