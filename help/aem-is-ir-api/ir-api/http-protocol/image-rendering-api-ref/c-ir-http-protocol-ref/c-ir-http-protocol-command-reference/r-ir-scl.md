...
Se `scl=` viene dopo `wid=` o `hei=` nell’URL, annulla tali comandi e `scl=` definisce le dimensioni dell&#39;immagine restituita dal server.
...
Può verificarsi in qualsiasi punto della richiesta. Ignorato se `wid=` o `hei=` dopo `scl=` nella sequenza di comando.
