...
L’immagine non viene imbottita se entrambe `wid=` e `hei=` è specificato e `wid`/ `hei` è diverso dalle proporzioni dell&#39;immagine.
...
Può verificarsi in qualsiasi punto della richiesta. Ridimensionamento dell’immagine con `wid=`, `hei=`oppure `scl=` non modifica il valore della risoluzione di stampa incorporato nell&#39;immagine di risposta. Ignorato se `scl=` si verifica dopo `wid=` o `hei=` nella sequenza di comando.
