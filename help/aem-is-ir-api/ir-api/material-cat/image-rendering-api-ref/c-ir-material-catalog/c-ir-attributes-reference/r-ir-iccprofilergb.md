---
title: IccProfileRgb
description: Profilo colore di output predefinito di RGB. Specifica il nome del profilo colore ICC da utilizzare per le immagini di risposta di RGB quando con icc= non è specificato alcuno spazio colore di output. Anche per alcuni valori di colore RGB specificati con vari comandi Image Rendering, come bgc= e color=.
...
Stringa di testo. Se specificato, deve essere valido `icc::Name` valore dalla mappa del profilo ICC di questo catalogo dei materiali o del catalogo predefinito o un percorso di file relativo a `attribute::RootPath`. Il profilo ICC di riferimento deve essere un profilo RGB.
