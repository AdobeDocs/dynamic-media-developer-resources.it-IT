...
Elemento regola richiesta. Uno o più sono facoltativi nell&#39; `<ruleset>` elemento .
...
Inoltre, `<rule>` gli elementi possono definire uno qualsiasi dei seguenti attributi in qualsiasi combinazione. Se specificato e la regola corrisponde correttamente, ignorano gli attributi catalogo corrispondenti per questa richiesta.
...
L&#39;attributo Expiration sostituisce solo il valore dell&#39;attributo predefinito; viene ignorato se alla richiesta viene applicato un `catalog::Expiration` valore specifico.
...
Se non `<expression>` viene specificato, qualsiasi percorso corrisponderà e `<substitution>` verrà aggiunto alla fine del percorso.

Se non `<substitution>` viene specificato, la sottostringa corrispondente viene rimossa.
