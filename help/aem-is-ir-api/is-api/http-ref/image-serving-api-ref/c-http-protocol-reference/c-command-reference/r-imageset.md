...
Set di immagini. Specifica un valore set di immagini da utilizzare per la generazione della risposta req=set.
...
Per applicare l’escape al valore e assicurarti che eventuali modificatori inclusi non vengano interpretati come parte della stringa di query URL, l’intero valore deve essere racchiuso tra parentesi graffe. Se il record catalogo è specificato nel percorso di rete, questo valore modificatore ha la precedenza `catalog::ImageSet` dal record principale. Per una descrizione della sintassi valida per i set di immagini, fare riferimento a `catalog::ImageSet` documentazione.

## Proprietà {#section-66e7bb7bf4664cbcac6f7ebb2f0d3a4f}

Attributo della richiesta. Facoltativo. Sostituzioni `catalog::ImageSet` da record principale.
...
Specifica il set di immagini da utilizzare con `req=set` richiesta:
