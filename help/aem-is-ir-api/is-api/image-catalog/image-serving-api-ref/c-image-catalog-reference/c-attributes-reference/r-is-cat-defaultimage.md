...
Può trattarsi di una voce di catalogo (incluso un modello) o di un percorso di file immagine relativo (con `attribute::RootPath`) o assoluto. Utile per sostituire le immagini mancanti con immagini predefinite.

## Proprietà {#section-b6d8193827c34e5f948792aba8b8daaf}

Stringa di testo. Se specificato, deve essere un `catalog::Id` valore valido in questo catalogo immagini o un percorso relativo (da `attribute::RootPath`) o assoluto a un file di immagine accessibile dal server immagini.
...
Ereditato da `default::DefaultImage` se non definito. Se definito ma vuoto, il comportamento predefinito dell&#39;immagine è disabilitato, anche se `default::DefaultImage` è definito.
