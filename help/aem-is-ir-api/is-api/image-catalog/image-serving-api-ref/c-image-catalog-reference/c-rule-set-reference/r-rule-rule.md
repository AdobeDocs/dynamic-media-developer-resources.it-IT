---
title: regola
description: Elemento della regola di richiesta. Una o più regole sono facoltative nel <ruleset> elemento.
...
Elemento della regola di richiesta. Una o più regole sono facoltative nel `<ruleset>` elemento.
...
Per ulteriori informazioni, fai riferimento alla descrizione dell’attributo di catalogo immagini corrispondente.

Gli attributi di scadenza sovrascrivono solo i valori attributo predefiniti. L&#39;override viene ignorato se un `catalog::Expiration` si applica alla richiesta.
...
La `<addressfilter>` viene applicato solo quando si verifica una corrispondenza e prima che vengano applicate le regole di query.
