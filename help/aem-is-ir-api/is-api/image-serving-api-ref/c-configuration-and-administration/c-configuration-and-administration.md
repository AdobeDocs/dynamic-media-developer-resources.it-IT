---
description: Utilizzare le informazioni contenute in questa sezione per configurare e amministrare il server immagini.
solution: Experience Manager
title: Configurazione e amministrazione
feature: Dynamic Media Classic,SDK/API
role: Developer,Admin,User
exl-id: fa02b6f7-777f-4c4d-a959-0f0d0fc96774
TQID: 'https://experienceleague.adobe.com/9nEIek0xNYqbNABwacqj-CFyFoiklKIlaV40-0LwtD4'
product_v2: id: fd1f54a9-f50c-467d-8956-cebbaf4f3eb8
feature_v2: id: a01bfd36-4ab8-4bf8-9dc0-5b45b890552e
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: c66ffd68-0f65-42bb-aa23-b4020f12e0bdid: ff6a42d2-313e-452e-93a6-792e4fad9ff8
topic_v2: id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: 2ff64206b7448a1a122696facd2669be68b6b9ff
workflow-type: tm+mt
source-wordcount: 57
ht-degree: 0%

---

# Configurazione e amministrazione{#configuration-and-administration}

Utilizzare le informazioni contenute in questa sezione per configurare e amministrare il server immagini.

* [File di configurazione del server](r-server-configuration-files/r-server-configuration-files.md)
* [Cache dei dati](c-data-caches/c-data-caches.md)
* [Impostazioni server](c-server-settings/c-server-settings.md)
* [Reindirizza in caso di errore](c-redirect-on-error.md)
* [Configurazione di SVG](c-configuring-svg.md)
* [Configurazione server](c-manual-server-configuration.md)
* [Definizione delle crittografie SSL](r-ssl-ciphers.md)
* [Supervisore server](c-server-supervisor.md)
* [Registrazione server](c-server-logging/c-server-logging.md)
* [Monitoraggio del server](c-server-monitoring/c-server-monitoring.md)
* [Gestione del contenuto](c-managing-content/c-managing-content.md)
