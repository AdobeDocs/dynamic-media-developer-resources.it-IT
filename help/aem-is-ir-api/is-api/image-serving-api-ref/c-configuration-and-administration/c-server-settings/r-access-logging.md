...
La cartella a cui appartiene il [!DNL Platform Server] scrive i file di registro. Può essere un percorso assoluto o un percorso relativo a *`install_folder`*. Il valore predefinito è [!DNL  *`install_folder`*/logs].
...
Prefisso nome per il file in cui vengono scritti i dati del registro di accesso. La data e il suffisso del file ( [!DNL  *`yyyy`*-*`mm`*-*`dd`*.log]) viene aggiunto alla stringa specificata. Il nome del file di log di accesso deve essere diverso da quello del file di log di traccia. Il valore predefinito è &quot; `access-`&quot;.

## TC::pattern - Access Log Pattern {#section-22775ea85cee444d8a7d7336a3b1feef}

Specifica il pattern di dati per [!DNL Platform Server] accedere ai record del registro. La stringa del pattern specifica le variabili che vengono sostituite con i relativi valori corrispondenti. Tutti gli altri caratteri nella stringa di pattern vengono letteralmente trasferiti nel record di log.

Per utilizzare l&#39;utilità di riscaldamento della cache, gli spazi devono essere utilizzati come separatori di campo. La [!DNL Platform Server] sostituisce tutti gli spazi e i caratteri &#39;%&#39; nei valori dei campi con `%20` e `%25`, rispettivamente.
...
<table id="table_7A07AFF34B5040A5B30F5735CFE9428F"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>[!DNL Platform Server] chiave cache (cartella/nome file cache). </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{CacheUse}r </span> </p> </td> 
   <td> <p>[!DNL Platform Server] parola chiave di gestione cache: <span class="codeph"> { RIUTILIZZATO | CREATO | AGGIORNATO | REMOTO | REMOTE_CREATED | REMOTE_AGGIORNATO | REMOTE_CACHE | CONVALIDATO | IGNORATO | UNDEFINED } </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>La <span class="codeph"> etag </span> valore dell'intestazione di risposta (firma MD5 dei dati di risposta). </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>Indirizzo IP del server peer nel cluster di cache che ha consegnato la voce di cache o "-" se <span class="codeph"> CacheUse </span> non è <span class="codeph"> REMOTE_CREATED </span> né <span class="codeph"> REMOTE_AGGIORNATO </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>Valore a cascata superiore di <span class="codeph"> req= </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>Il tempo necessario [!DNL Platform Server] inviare la risposta dopo la scrittura dei dati nel flusso di output. </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{Size}r </span> </p> </td> 
   <td> <p>Simile <span class="codeph"> %B </span>, ma include i valori per le risposte 304 (non modificate). </p> </td> 
  </tr> 
  <tr> 
