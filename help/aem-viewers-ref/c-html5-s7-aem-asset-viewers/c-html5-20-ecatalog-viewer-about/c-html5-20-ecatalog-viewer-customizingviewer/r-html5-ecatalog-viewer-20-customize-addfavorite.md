...
>[!NOTE]
>
>Questo pulsante supporta sia i selettori di attributi `state` che `selected`, che possono essere utilizzati per applicare interfacce diverse a stati di pulsante diversi. In particolare, `selected='true'` corrisponde allo stato in cui un utente può aggiungere una nuova icona Preferiti selezionando o toccando. L&#39;attributo `selected='false'` corrisponde alla normale modalità operativa in cui un utente può eseguire zoom, scorrimento e scambio di pagine.
