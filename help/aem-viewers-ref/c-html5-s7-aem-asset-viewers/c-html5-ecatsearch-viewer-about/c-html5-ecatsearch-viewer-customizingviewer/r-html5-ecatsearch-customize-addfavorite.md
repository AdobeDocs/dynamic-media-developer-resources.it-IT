...
description: La posizione del pulsante Aggiungi preferiti è completamente gestita dal menu Preferiti.
...
La posizione del pulsante Aggiungi preferiti è completamente gestita dal menu Preferiti.
...
>[!NOTE]
>
>Questo pulsante supporta sia `state` e `selected` selettori di attributi, che possono essere utilizzati per applicare interfacce diverse a stati di pulsante diversi. In particolare: `selected='true'` corrisponde allo stato in cui un utente può aggiungere una nuova icona Preferiti selezionando o toccando. considerando che `selected='false'` corrisponde alla modalità operativa normale quando un utente è in grado di eseguire zoom, panning e scambio di pagine.
...
Esempio: per impostare un pulsante Aggiungi preferito di 28 x 28 pixel e visualizzare un&#39;immagine diversa per ciascuno dei quattro stati del pulsante, se selezionato o non selezionato.
