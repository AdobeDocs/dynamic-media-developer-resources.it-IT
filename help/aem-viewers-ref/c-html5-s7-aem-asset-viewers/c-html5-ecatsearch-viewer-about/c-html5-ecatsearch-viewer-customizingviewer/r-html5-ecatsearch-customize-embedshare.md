...
>[!NOTE]
>
>La casella combinata supporta il selettore di attributi `expanded` con valori possibili di `true` e `false`. Il valore `true` viene utilizzato quando nella casella combinata viene visualizzata una delle dimensioni di incorporamento predefinite, pertanto dovrebbe essere occupata tutta la larghezza disponibile. Il valore `false` viene utilizzato quando nella casella combinata è selezionata l&#39;opzione di dimensione personalizzata, pertanto deve essere ridotto per consentire lo spazio per i campi di input personalizzati per larghezza e altezza.
