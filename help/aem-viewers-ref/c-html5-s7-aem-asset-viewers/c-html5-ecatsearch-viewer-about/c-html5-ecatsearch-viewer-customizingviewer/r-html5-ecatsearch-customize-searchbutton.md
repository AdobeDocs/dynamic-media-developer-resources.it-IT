...
>[!NOTE]
>
>Questo pulsante supporta i selettori `state` e `selected` attributo, che possono essere utilizzati per applicare interfacce diverse a diversi stati del pulsante.
>
>In particolare, `selected='false'` corrisponde allo stato iniziale del pulsante di scorrimento e `selected='true'` allo stato in cui è attivo il pannello di ricerca.
