...
Inoltre, è importante preservare la seguente dichiarazione CSS dal CSS del visualizzatore predefinito in quanto necessaria per la funzionalità di zoom in linea:
...
Quando crei CSS personalizzati, ricorda che il visualizzatore assegna `.s7flyoutviewer` all&#39;elemento DOM contenitore. Se utilizzi un file CSS esterno passato con `style=` comando, utilizza `.s7flyoutviewer` Classe come classe padre nel selettore discendente per le regole CSS. Se esegui stili incorporati nella pagina web, qualifica anche questo selettore con un ID dell’elemento DOM contenitore come segue:
...
Per facilitare la creazione di CSS adattabili, il visualizzatore supporta marcatori CSS che assegnano in modo dinamico classi CSS speciali all’elemento contenitore di visualizzatori di primo livello. Le assegnazioni si basano sulle dimensioni del visualizzatore in fase di esecuzione e sul tipo di input utilizzato sul dispositivo corrente.
...
L’utilizzo dei marcatori CSS è il modo più flessibile per creare CSS adattabili. Consente di eseguire il targeting non solo delle dimensioni dello schermo del dispositivo, ma anche delle dimensioni effettive del visualizzatore, il che può essere utile per i layout reattivi di pagine web.
