...
description: Personalizzazione del visualizzatore zoom in linea
keywords: responsive
...
Tutte le personalizzazioni visive e la maggior parte delle personalizzazioni comportamentali vengono eseguite creando un CSS personalizzato.
...
Inoltre, è importante mantenere la seguente dichiarazione CSS dal CSS visualizzatore predefinito in quanto è necessaria per la funzionalità di zoom in linea:
...
Quando crei CSS personalizzato, ricorda che il visualizzatore assegna `.s7flyoutviewer` al relativo elemento DOM contenitore. Se utilizzi un file CSS esterno passato con `style=` comando, utilizza `.s7flyoutviewer` come classe padre nel selettore discendente per le regole CSS. Se esegui stili incorporati nella pagina web, qualifica anche questo selettore con un ID dell’elemento DOM del contenitore come segue:

`#<containerId>.s7flyoutviewer`

## Creazione di CSS reattivo {#section-c1e74f5114ad418884ca1c95f5ea5b63}

È possibile eseguire il targeting di dispositivi diversi e incorporare dimensioni in CSS per rendere il contenuto visualizzato in modo diverso, a seconda del dispositivo di un utente o di un particolare layout di pagina web. Questa capacità include, ma non è limitata a, layout di pagina web diversi, dimensioni degli elementi dell’interfaccia utente e risoluzione dei disegni.
...
Per facilitare la creazione di CSS dinamici, il visualizzatore supporta i marcatori CSS che assegnano dinamicamente classi CSS speciali all’elemento contenitore del visualizzatore di primo livello. Le assegnazioni si basano sulle dimensioni del visualizzatore in fase di esecuzione e sul tipo di input utilizzato nel dispositivo corrente.
...
L’utilizzo dei marcatori CSS è il modo più flessibile per creare CSS progettati responsive. Consente di rivolgersi non solo alle dimensioni dello schermo del dispositivo, ma anche alle dimensioni effettive del visualizzatore, il che può essere utile per i layout reattivi progettati per le pagine web.
...
## Spunti CSS {#section-b0af39db1af74561aea9fddcc8cdc2c7}
...
Con un approccio classico allo stile, il CSS avrebbe un riferimento separato a un singolo file di immagine sul server per ogni stato dell’elemento dell’interfaccia utente. Di seguito è riportato un esempio di CSS per la formattazione di un pulsante di scorrimento:
...
Di seguito è riportata la documentazione di riferimento sugli elementi dell’interfaccia utente applicabile al visualizzatore a comparsa:
