...
# Personalizzazione del visualizzatore per file multimediali diversi{#customizing-mixed-media-viewer}
...
Quando create CSS personalizzato, tenete presente che il visualizzatore assegna la `.s7mixedmediaviewer` classe al relativo elemento DOM contenitore. Se utilizzate un file CSS esterno passato con `style=` comando, utilizzate la `.s7mixedmediaviewer` classe come classe principale nel selettore discendente per le regole CSS. Se state utilizzando stili incorporati nella pagina Web, qualificate anche questo selettore con un ID dell&#39;elemento DOM contenitore come segue:
