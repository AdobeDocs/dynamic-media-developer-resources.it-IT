...
È possibile localizzare la descrizione del pulsante Chiudi e il titolo della finestra di dialogo. Per ulteriori informazioni, consultate [Localizzazione degli elementi](../../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/r-html5-video-viewer-20-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) dell&#39;interfaccia utente.

Esempio: per impostare l’intestazione della finestra di dialogo con la spaziatura, l’icona da 24 x 17 pixel, il titolo in grassetto da 16 pt e il pulsante Chiudi da 28 x 28 pixel posizionato a 2 pixel dalla parte superiore e a 2 pixel dal lato destro del contenitore della finestra di dialogo:
...
Il piè di pagina della finestra di dialogo è costituito dai pulsanti &quot;cancel&quot; e &quot;send email&quot;. Il contenitore piè di pagina è controllato dal seguente selettore di classe CSS:
...
Le descrizioni dei pulsanti possono essere localizzate. Per ulteriori informazioni, consultate [Localizzazione degli elementi](../../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/r-html5-video-viewer-20-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) dell&#39;interfaccia utente.
...
All&#39;interno di un contenitore controllato da
...
Esempio: per impostare un modulo di una finestra di dialogo con dieci pixel di spaziatura per ogni riga:
...
Le etichette delle finestre di dialogo possono essere localizzate. Per ulteriori informazioni, consultate [Localizzazione degli elementi](../../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/r-html5-video-viewer-20-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) dell&#39;interfaccia utente.
...
>[!NOTE]
>
>Il contenitore dei campi di input supporta il selettore `state` di attributi opzionale. È impostato su `verifyerror` quando l&#39;utente commette un errore nel formato dei dati di input e la convalida in linea non riesce. Questo selettore di attributi può essere utilizzato per evidenziare l&#39;input dell&#39;utente errato nel modulo.

La maggior parte dei campi di input che si estendono dall’etichetta a sinistra fino al bordo destro del corpo della finestra di dialogo (che include i campi &quot;from&quot; e &quot;message&quot;) sono controllati da:
...
Il campo di input &quot;A&quot; è più stretto perché alloca spazio per il pulsante &quot;Rimuovi e-mail&quot; a destra. È controllato dal seguente selettore di classe CSS:
...
Esempio: per impostare un modulo con un bordo grigio di un pixel e nove pixel di spaziatura intorno a tutti i campi di input; per i campi con errore di convalida, avere lo stesso bordo in rosso, avere un campo di input &quot;A&quot; largo 250 pixel e il resto dei campi di input largo 300 pixel:
...
Il campo di immissione dei messaggi e-mail è inoltre controllato con:

```
.s7videoviewer .s7emaildialog .s7dialogmessage
```

Questa classe consente di impostare proprietà specifiche per l&#39; `TEXTAREA` elemento sottostante.
...
Esempio: per impostare un messaggio e-mail con un&#39;altezza di 50 pixel e utilizzare il ritorno a capo `break-word` automatico:
...
Il pulsante &quot;Remove&quot; consente a un utente di rimuovere altri destinatari dal modulo e-mail. È controllato dal seguente selettore di classe CSS:
...
Esempio: per impostare un pulsante &quot;Remove&quot; su 25 x 25 pixel e utilizzare un’immagine diversa per ogni stato:
...
Il contenuto condiviso viene visualizzato nella parte inferiore del corpo della finestra di dialogo e include una miniatura, un titolo, un URL di origine e una descrizione. È avvolto in contenitore controllato con:
...
La `background-image` proprietà è impostata dalla logica del componente.
...
Esempio: per impostare una miniatura di 90 x 60 pixel e allinearla in alto con dieci pixel di spaziatura:
...
Esempio: per impostare un titolo di contenuto in modo che utilizzi il font grassetto e abbia un margine di dieci pixel:
...
>[!NOTE]
>
>Questo messaggio supporta il selettore `state` di attributi con i seguenti valori possibili: `verifyerror`, `senderror`, e `sendsuccess`. `verifyerror` è impostato quando viene visualizzato un messaggio a causa di un errore di convalida dell&#39;input in linea; è `senderror` impostato quando un servizio e-mail di back-end segnala un errore; viene `sendsuccess` impostato quando l’e-mail viene inviata correttamente. In questo modo è possibile formattare il messaggio in modo diverso a seconda dello stato della finestra di dialogo.

Il messaggio di errore può essere localizzato. Per ulteriori informazioni, consultate [Localizzazione degli elementi](../../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/r-html5-video-viewer-20-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) dell&#39;interfaccia utente.
...
Esempio: per impostare la casella di scorrimento della barra di scorrimento di 28 x 45 pixel, il margine superiore e inferiore è di 10 pixel e per ogni stato è disponibile un’immagine diversa:
