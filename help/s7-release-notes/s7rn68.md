---
description: Note aggiornate sulla versione che descrivono le funzioni, le correzioni e i miglioramenti implementati in Adobe Scene7 Publishing System 6.8, componente della soluzione di Adobe Experience Manager  nell’Adobe Marketing Cloud .
seo-description: Note aggiornate sulla versione che descrivono le funzioni, le correzioni e i miglioramenti implementati in Adobe Scene7 Publishing System 6.8, componente della soluzione di Adobe Experience Manager  nell’Adobe Marketing Cloud .
...
Note aggiornate sulla versione che descrivono le funzioni, le correzioni e i miglioramenti implementati in Adobe Scene7 Publishing System 6.8, componente della soluzione di Adobe Experience Manager  nell’Adobe Marketing Cloud .

## Note sulla versione di Scene7 Publishing System 6.8 (maggio 2014) {#topic-4517aab837ea4d6d8204425958860e5a}

Note aggiornate sulla versione che descrivono le funzioni, i miglioramenti e le correzioni più recenti per [!DNL Adobe Scene7 Publishing System] 6.8 parti della soluzione di Adobe Experience Manager  in [!DNL Adobe Marketing Cloud].
...
* Integrazione di rapporti video Adobe  Analytics per i visualizzatori video HTML5 e per i visualizzatori per file multimediali diversi.
* Tracciamento degli eventi per più visualizzatori HTML5 sulla stessa pagina.
* Aggiunto `SetIndicator` ai visualizzatori HTML5 per zoom e HTML5 per file multimediali diversi per rappresentare visivamente i campioni sui dispositivi mobili.
* È stata aggiunta `getComponent` l’API per consentire l’accesso esterno ai componenti visualizzatore HTML5.
...
*  handler Analytics ai visualizzatori HTML5 per supportare il tracciamento di terze parti.
...
Consultate anche la guida [di riferimento dei visualizzatori di](/help/aem-viewers-ref/t-installing-the-viewers.md)Scene7.
...
* Convertite le e-mail con password dimenticata da HTML a testo normale per evitare il blocco degli elenchi.
* È stato eliminato il ruolo predefinito.
* Oltre alla possibilità di caricare contenuti in Scene7 Publishing System tramite FTP (Server Directory Upload) e URL di caricamento, ora potete anche caricare contenuti tramite HTTP POST. Per ulteriori informazioni sull’utilizzo di HTTP POST, consultate [Image Production System API](/help/aem-ips-api/c-overview.md) .
...
* Il visualizzatore video HTML5 non consente l’accesso per sbloccare il plug-in Flash.
...
* Quando inizializzate le impostazioni della società, Scene7 Publishing System invia l’opzione duplicata `<defaultValue>` nell’API `<createMetadataField>`.

* Il plug-in Flash e Scene7 Publishing System si bloccano a causa di un numero elevato di cartelle che non dispongono di memoria Flash sufficiente.
* Quando rimuovete il valore per il campo Data/Ora definito dall’utente, Scene7 Publishing System non invia una richiesta a IPS (Image Publishing System).
...
* Informazioni sull’account FTP di Scene7 mancanti nel messaggio e-mail di benvenuto per un account di prova
...
* L’etichetta Società non viene visualizzata completamente nella pagina di configurazione di Adobe  Analytics.
* Impossibile importare i metadati.

## Problemi noti {#section-b2608177928b4174b080d89c938bb0d3}

* L’aggiornamento di maggio 2014 di Adobe Illustrator CC (Creative Cloud) e Adobe InDesign CC dovrebbe introdurre incompatibilità con i plug-in che creano il file FXG di Scene7. Queste incompatibilità interessano gli utenti della soluzione Scene7 Web-stampa. Se usate Scene7 Web-stampa, non aggiornate Illustrator e InDesign con questo aggiornamento di Creative Cloud. Al contrario, continuate a utilizzare la versione precedente a maggio 2014. Adobe Systems prevede di ripristinare la compatibilità della creazione di file FXG per Scene7 con le nuove versioni di Illustrator e InDesign nell’autunno 2014.
...
* La sillabazione del testo non sarà più supportata nella release autunnale di Scene7 6.9. I dizionari di sillabazione definiscono il modo in cui le parole possono essere suddivise e suddivise in sillabe per ottimizzare lo scorrimento del testo.
...
* In questa versione i banner dinamici (Flash) sono stati ritirati. L&#39;adozione di questa funzione è stata limitata a causa della comparsa di altre tecnologie per la creazione, la gestione e la distribuzione di banner Web personalizzati. La funzionalità per banner dinamici di Scene7 (Flash) continuerà a funzionare sul lato server ma non sarà più supportata.
* Promemoria: Il server di metadati e l’utilizzo delle relative API sono stati ufficialmente dissupportati a partire dalla versione precedente di Scene7.
* Le estensioni di Scene7 Creative Suite (pannelli Flash) verranno ritirate alla fine del 2014. L&#39;ulteriore sviluppo dei flussi di lavoro per l&#39;assimilazione dei contenuti si concentra  Adobe Experience Manager Dynamic Media e Creative Cloud. Per Scene7, le funzioni di caricamento ed esportazione in Scene7 Publishing System rimarranno e potranno essere utilizzate.
...
* Sezione dell’account FTP di Scene7 nel modello e-mail di benvenuto per gli utenti di Media Portal.
...
Consultate [https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html)
...
* Problema corretto: `selectSwatch` non viene attivato quando incorporato nella `initViewer()` funzione.
...
* Problema corretto: L&#39;icona Condivisione social network non si dissolve con lo `controlbar` zoom attivato.
...
* È stato aggiunto il supporto per `initialBitRate` il modificatore per selezionare il bitrate utilizzato per la riproduzione iniziale su desktop.
...
* Problema corretto: Protocollo HTTPS che genera un avviso di protezione su IE8 utilizzando la riproduzione Flash.
* Problema corretto: Il video non viene ridimensionato correttamente in modalità a schermo intero su Safari 5 mediante la riproduzione Flash.
