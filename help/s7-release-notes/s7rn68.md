...
description: Note aggiornate sulla versione che descrivono le funzioni, i miglioramenti e le correzioni più recenti di Adobe Scene7 Publishing System 6.8, parte della soluzione Adobe Experience Manager in Adobe Experience Cloud.
...
Note aggiornate sulla versione che descrivono le funzioni, i miglioramenti e le correzioni più recenti della soluzione Adobe Experience Manager in Adobe Experience Cloud per Adobe Scene7 Publishing System 6.8.
...
* Visualizzatori HTML5 per utilizzare input CSS e marcatori dimensione per controllare la visualizzazione su desktop e dispositivi mobili.

Sono ora supportati i seguenti elementi:

* Integrazione dei rapporti video di Adobe Analytics per visualizzatore video HTML5 e visualizzatore file multimediali diversi.
...
* Parametri di configurazione del visualizzatore HTML5 passati come un singolo oggetto JSON.
* Sottotitoli WebVTT da passare come oggetto JSON, eliminando la necessità di configurare il set di regole.
* Modelli JavaScript in HTML5 eCatalog Viewer per il collegamento di mappe immagine a pagine esterne.
* Collegamento a pagine all&#39;interno del visualizzatore eCatalog di HTML5.
...
* Gestore di Analytics per i visualizzatori HTML5 per il supporto del tracciamento di terze parti.
...
* Oltre alla possibilità di caricare i contenuti in Scene7 Publishing System tramite il caricamento di directory server (ftp) e URL, ora è possibile caricare i contenuti anche tramite HTTP POST. Per ulteriori informazioni sull&#39;utilizzo di HTTP POST, vedere [Image Production System API](/help/aem-ips-api/c-overview.md).
...
* Impossibile caricare i visualizzatori HTML5 a causa di un UserAgent non riconosciuto.
* Il trascinamento del mouse del visualizzatore eCatalog di HTML5 non funziona in Internet Explorer con una pagina contenente mappe immagine.
* Visualizzatore video HTML5 impedisce l&#39;accesso allo sblocco del plug-in Flash.
...
* autoResize non deve trovarsi nel codice di incorporamento/URL della copia del visualizzatore a comparsa di HTML.
* Nessun modificatore videoserverurl nell&#39;URL di copia del visualizzatore video HTML5.
...
* Il plug-in Flash e Scene7 Publishing System si sono bloccati a causa della mancanza di memoria flash in molte cartelle.
...
* Caratteri speciali venivano codificati in un messaggio e-mail in formato HTML &quot;Dimentica password&quot;, causando la visualizzazione errata della password.
* Informazioni account FTP Scene7 mancanti nell&#39;e-mail di benvenuto per un account di prova
...
* L’aggiornamento di maggio 2014 di Adobe Illustrator e Adobe InDesign dovrebbe introdurre incompatibilità con i plug-in che creano Scene7 FXG. Queste incompatibilità riguardano gli utenti della soluzione Scene7 Web-to-Print. Se si utilizza Scene7 Web-to-Print, non aggiornare Illustrator e InDesign a questo aggiornamento Creative Cloud. Continua invece a utilizzare la versione precedente a maggio 2014. Adobe Inc prevede di ripristinare la compatibilità della creazione di Scene7 FXG con le versioni più recenti di Illustrator e InDesign nell’arco temporale di autunno 2014.
...
* In questa versione i banner dinamici (Flash) diventeranno obsoleti. L’adozione di questa funzione è stata bassa, in quanto sono emerse altre tecnologie per la creazione, la gestione e la distribuzione di banner personalizzati per i siti web. La funzionalità Banner dinamico (Flash) di Scene7 continua a funzionare sul lato server, ma non è più supportata.
* Promemoria: il server di metadati e l’utilizzo delle API associate sono stati ufficialmente ssupportati dalla precedente versione di Scene7.
* Le estensioni di Scene7 Creative Suite (Pannelli Flash) sono state dichiarate obsolete alla fine del 2014. Ulteriori sviluppi nei flussi di lavoro per l’acquisizione dei contenuti si concentrano su Adobe Experience Manager Dynamic Media e Creative Cloud. Per Scene7, le funzionalità di caricamento ed esportazione rimangono disponibili in Scene7 Publishing System e possono essere utilizzate al loro posto.
...
* Sezione account FTP Scene7 nel modello e-mail di benvenuto per l’utente di Media Portal.
...
* Risolto: il protocollo HTTPS genera un avviso di sicurezza su IE8 utilizzando la riproduzione Flash.
* Fisso: il video non viene ridimensionato correttamente in modalità a schermo intero su Safari 5 utilizzando la riproduzione Flash.
...
* Risolto: supporto HTTPS per i visualizzatori HTML5 e AS3.
* È stato aggiunto il supporto reattivo al codice di incorporamento dei visualizzatori HTML5.
